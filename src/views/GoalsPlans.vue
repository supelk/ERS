...
onMounted(async () => {
  loading.value = true
  try {
    await taskStore.autoCheckOverdue()
    await taskStore.fetchTasks()
    sectionOptions.value = await taskStore.getSectionOptions()
    await loadGoals()
  } catch (e) {
...
async function handleAdvance(task: PracticeTask) {
  ...
  try {
    await taskStore.updateTaskStatus(task.task_id, nextStatus)
  } catch (e) {
          ...
          <NGrid :cols="4" :x-gap="12">
            <NGi v-for="(statusKey, _) in (['未开始','进行中','已完成','逾期'] as TaskStatus[])" :key="statusKey">
              <TaskColumn
                ...
                :section-labels="taskStore.sectionNames"
